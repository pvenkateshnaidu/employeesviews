 ...
 */
	...
	require_once THEME_PATH . "/include/2checkout/payment-campaigns.php";
		/* ============== education   ============ */
require_once THEME_PATH . '/include/custom-functions.php';
	...
	function listingpro_style() {
		...
		wp_enqueue_style('Icofont', THEME_DIR . '/assets/lib/icofont/icofont.min.css');
...
*/ 
 ...
 function wpd_search_template( $template ) {
    if( ! have_posts() ) {
        $template = locate_template( '/404-error'  );
    }
    return $template;
}
...
function codecanal_forgot_mail_contnet($message, $key) {
    ...
    return $message . "- <a href=' " . $reset_url . " '> Reset Password </a>";
}
...
function ajax_cf7_populate_values() {
        ...
        // read the CSV file in the $regions_countries_links array

	$regions_countries_links = array();
	$uploads_folder = wp_upload_dir()['basedir'];
	$file = fopen($uploads_folder.'\region_country_imm_link.csv', 'r');
	...
	while (($line = fgetcsv($file)) !== FALSE) {
		...
		$regions_countries_links[$line[0]][$line[1]][] = $line[2];
	...
	$return_array = array(
            'regions' => array_keys($regions_countries_links),
            'countries' => array(),
            'links' => array(),
	...
	$region = key_exists('region', $_POST) ? $_POST['region'] : false;
	$country = key_exists('country', $_POST) ? $_POST['country'] : false;
	$link = key_exists('link', $_POST) ? $_POST['link'] : false;

        // populate the $return_array with the necessary values

	if ($region) {
		$return_array['current_region'] = $region;
		$return_array['countries'] = array_keys($regions_countries_links[$region]);
	    if ($country) {
		$return_array['current_country'] = $country;
		$return_array['links'] = $regions_countries_links[$region][$country];
		if ($link) {
	            $return_array['current_link'] = $link;
	        }
            }
        }
...
/* Rating Ajax Pagination */
add_action( 'wp_ajax_review-pagination-load-posts', 'cvf_review_pagination_load_posts' );
// We allow non-logged in users to access our pagination
add_action( 'wp_ajax_nopriv_review-pagination-load-posts', 'cvf_review_pagination_load_posts' ); 
function cvf_review_pagination_load_posts() {
    ...
    if(isset($_POST['page'])){
     ...
     /*   $all_blog_posts = $wpdb->get_results($wpdb->prepare("
            SELECT * FROM " . $table_name . " WHERE post_type = 'edu-reviews' AND post_status = 'publish' ORDER BY post_date DESC LIMIT %d, %d", $start, $per_page ) ); */
        
        // At the same time, count the number of queried posts
       /* $count = $wpdb->get_var($wpdb->prepare("
            SELECT COUNT(ID) FROM " . $table_name . " WHERE post_type = 'edu-reviews' AND post_status = 'publish'", array() ) );*/
        
if(isset( $_POST['category']) && !empty($_POST['category']))
{
	    $category = sanitize_text_field($_POST['category']);

        $getcount = get_posts(array(

        'fields' => 'ids',
        	 'posts_per_page' 	=> -1,
       'post_type' => 'edu-reviews',
       'meta_key'		=> 'millionmaker_category',
	'meta_value'	=> $category
        ...
        $count =count($getcount );
         ...
         $all_blog_posts = new WP_Query(
            array(
  			...
  			// 'meta_key'		=> 'millionmaker_category',
			//'meta_value'	=> $category,
                'meta_key'		=> 'date_of_review',       	
				  'orderby'           => 'meta_value_num',
				  'order'             => 'DESC',
                 'meta_query' => array(
                 	array(
                    'key' => 'millionmaker_category',
                    'value' =>$category,
                )
                 ),
   ...
   /*     $count = new WP_Query(
        ...
        $all_blog_posts->the_post(); // This is where the post's data is set up

            
            // Set the desired output into a variable
            $msg .= get_template_part('template-parts/all-reviews-content');
...
add_action( 'wp_ajax_review-cat-count', 'review_cat_count' );
// We allow non-logged in users to access our pagination
add_action( 'wp_ajax_nopriv_review-cat-count', 'review_cat_count' ); 
function review_cat_count()
{
...
add_action("wp_ajax_get_virtual_links", "get_virtual_links");
add_action("wp_ajax_nopriv_get_virtual_links", "get_virtual_links");

function get_virtual_links()
{

$servername = "localhost";
$username = "admin_link_generationdbuser";
$password = "3_@qIDrt8l2x";
$dbname = "admin_link_generationdb";
...
if(!empty($_GET["Virtual_Office_Region_ID"])){ 

    // Fetch state data based on the specific country 

    $query = "SELECT * FROM virtual_office_country WHERE Virtual_Office_Region_ID = ".$_GET['Virtual_Office_Region_ID']." ORDER BY Virtual_Office_Country_Name ASC"; 
    ...
    if($results){ 
       ...
       foreach($results as $row) {
            ...
            echo '<option value="'.$row->Virtual_Office_Country_ID.'">'.$row->Virtual_Office_Country_Name.'</option>'; 
...
elseif(!empty($_GET["Virtual_Office_Country_ID"])){ 

    // Fetch city data based on the specific state 

    //$query = "SELECT * FROM visa_links WHERE Visa_Country_ID = ".$_POST['Visa_Country_ID']." ORDER BY Visa_Links_Name ASC"; 

	$query = "SELECT * FROM virtual_office_city WHERE Virtual_Office_Country_ID = ".$_GET['Virtual_Office_Country_ID']." ORDER BY Virtual_Office_City_Name ASC"; 
    ...
    if($results){ 
       ...
       foreach($results as $row) {
            ...
            echo '<option value="'.$row->Virtual_Office_City_ID.'">'.$row->Virtual_Office_City_Name.'</option>'; 

          // echo '<a href="'.$row['Virtual_Office_City_Name'].'" target="_blank">Explore More</a>'; 

        } 

    }else{ 

        echo '<option value="">Link not available</option>'; 

    } 

} 

elseif(!empty($_GET["Virtual_Office_City_ID"])){ 

    // Fetch city data based on the specific state 

    //$query = "SELECT * FROM visa_links WHERE Visa_Country_ID = ".$_POST['Visa_Country_ID']." ORDER BY Visa_Links_Name ASC"; 

	$query = "SELECT * FROM virtual_office_links WHERE Virtual_Office_City_ID = ".$_GET['Virtual_Office_City_ID']." ORDER BY Virtual_Office_Links_Name ASC"; 
    ...
    if($results){ 
       ...
       // echo '<option value="">Select Link</option>'; 

        foreach($results as $row) {

          //  echo '<option value="'.$row['Virtual_Office_City_ID'].'">'.$row['Virtual_Office_City_Name'].'</option>'; 

           echo '<a href="'.$row->Virtual_Office_Links_Name.'" target="_blank">Explore More</a>'; 
...
{
    if ( is_singular( 'mmeducation' )||is_singular( 'company-formation' )||is_page()) // If we are on a single post page from my my_post_type, dequeue the script
    {
    	
       	 wp_dequeue_script( 'ajax-single-ajax' );
           wp_dequeue_script( 'ajax-approvereview-script' );
             wp_dequeue_script( 'stripejs' );
              wp_dequeue_script( 'listingpro_home_map' );
              wp_dequeue_script( 'ajax-login-script' );
                 wp_dequeue_script( 'review-submit-ajax' );  
                    wp_dequeue_script( 'lp-icons-colors' );

                  wp_dequeue_script('Main');
                  wp_dequeue_script( 'Mapbox');
                  wp_dequeue_script( 'Mapbox-leaflet');
                  wp_dequeue_script( 'Pricing');
                  wp_dequeue_script( 'dyn-location-js');                   
                   wp_dequeue_script( 'bootstrap-datetimepicker');
                       wp_dequeue_style('bootstrap-datetimepicker-css');
                        wp_dequeue_style('MapBox');                    
                    wp_dequeue_style('dynamiclocation');
                       wp_dequeue_style('mourisjs');
                  wp_dequeue_script( 'bootstrap-moment'); 
                   wp_dequeue_script( 'raphelmin'); 
                    wp_dequeue_script( 'morisjs'); 
...
add_action('wp_ajax_nopriv_purchase_request_email', 'purchase_request_email_function');
add_action('wp_ajax_purchase_request_email', 'purchase_request_email_function');
function purchase_request_email_function(){

$htmlData = $_REQUEST['htmlData'];
$intialValue = $_REQUEST['intialValue'];
...
$body ='';
$body ='Company Formation '.get_the_title( $pageId ).'';
$body .='Incorporation Fees $'.number_format($intialValue,2);
$body .='Other Services';
$body .=$htmlData;
$body .='Total amount : $'.number_format($totalAmount,2);
$body .='';
$body .="Full Name : ". $_REQUEST['fname']." ".$_REQUEST['lname'];
$body .="Email : ". $_REQUEST['pemail'];
$body .="Telephone : ". $_REQUEST['telephone'];
$body .="Country Name : ". $_REQUEST['countryName'];
$body .="Business Type : ". $_REQUEST['businesstype'];
$body .="Ip Tracking : ". $_REQUEST['IpAddress'];

		$subject = 'Company Formation Request';
        $emailto ="saint.neo1101@gmail.com";      
    ...
    $headers = array('Content-Type: text/html; charset=UTF-8');
	  ...
	  if( is_user_logged_in() )
	  {
		...
		// Alternative for getting current user ID
		// Update current user's first name
		$data ='<br/>Total amount : $'.number_format($totalAmount,2);
		update_user_meta( $current_user_id, 'purchase_requests_mm',$htmlData.' '. $data );
	  }
         $respose= wp_mail( $emailto, $subject, $body );
         return $respose;
die();
}
function iptracking_customcode() {
		...
		$trackingInfo .= __('The user filled the form on:','apa_cf7_lead_tracking') . ' ' . get_permalink(get_the_id()) . $lineBreak;
